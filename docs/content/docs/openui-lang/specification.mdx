...
These are the most frequent mistakes LLMs make when generating OpenUI Lang.
...
Each identifier can only be assigned once. If the same name is used twice, the second assignment is dropped.

```text
// Wrong — "card" assigned twice, second is dropped
card = Card([title1])
card = Card([title2])
