...
export interface QueryManager {
  ...
  /** Fire a mutation with evaluated args. Called on button click. */
  fireMutation(
    statementId: string,
...
export function createQueryManager(transport: Transport | null): QueryManager {
  ...
  function rebuildSnapshot(): boolean {
    ...
    // Preserve identity if content is unchanged (JSON comparison handles
    // array values like __loading that are always new references)
    const outJson = JSON.stringify(out);
    if (outJson === JSON.stringify(snapshot)) return false;
    snapshot = out;
    return true;
  }
