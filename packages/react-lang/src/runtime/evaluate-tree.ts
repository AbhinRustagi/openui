 ...
 *   - StateRef + reactive schema → emits ReactiveAssign for useBoundField() at render time
...
function evaluatePropValue(
  ...
  if (isASTNode(value)) {
    ...
    // Do NOT resolve to BoundField here — FormNameContext isn't available yet.
    // Components call useBoundField() at render time for correct form scope.
  ...
  // Components call useBoundField() which resolves strings via form state.
